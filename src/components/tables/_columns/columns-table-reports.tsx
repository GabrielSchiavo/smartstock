...
export const columnsTableReportValidity: ColumnDef<ValidityReportResponse>[] = [
  ...
  {
    accessorKey: 'name',
    header: ({ column }) => <DataTableColumnHeader column={column} title="Nome do Produto" />,
    meta: {
      title: 'Nome',
    } as ColumnMetaProps,
  },
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title="Dias para Vencer" />,
    cell: ({ row }) => {
      const days = row.original.daysUntilExpiry;
      const daysString = days.toString();

      if (days <= 0) {
        return <span className="bg-muted rounded-sm px-3 py-1 text-sm uppercase">Vencido</span>;
      } else {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if (status === ValidityStatusType.EXPIRED) {
        return (
          <Badge className="bg-red-500/15 text-sm text-red-600 dark:text-red-500">Vencido</Badge>
        );
