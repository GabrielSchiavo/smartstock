...
export function DynamicComboboxReceiver({
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (updatedResults.success) {
            ...
            toast.error(
              updatedResults.message || "Falha ao atualizar os recebedores",
              {
                description:
                  updatedResults.error ||
                  "Não foi possível carregar a lista de recebedores atualizada",
              }
            );
          }
        } else {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Ocorreu um erro ao criar o recebedor";
  ...
  const handleDelete = async (optionId: string, optionName: string) => {
    startTransition(async () => {
      try {
        const { isUsed, message } = await checkReceiverUsage(optionName);
