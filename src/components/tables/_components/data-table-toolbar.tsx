...
export function DataTableToolbar<TData>({
  ...
  return (
    <div className={`flex w-full items-center justify-between gap-4 sm:gap-6`}>
      <div
        className={`flex items-center justify-center gap-2 ${filters.length > 0 && 'flex-wrap'}`}
      >
        ...
        {toolTip ? (
          <ToolTipHelp>
            <TooltipItem>
              <span className="min-h-4 min-w-4 rounded-sm bg-emerald-600 dark:bg-emerald-500"></span>
              <p className="text-sm">
                <span className="font-semibold">Dentro da Validade</span> - Produto apto para
                uso/venda.
              </p>
            </TooltipItem>
            <TooltipItem>
              <span className="min-h-4 min-w-4 rounded-sm bg-yellow-600 dark:bg-yellow-500"></span>
              <p className="text-sm">
                <span className="font-semibold">Próximo do Vencimento</span> - Validade expira em
                até {daysDefaultUntilExpiry} dias.
              </p>
            </TooltipItem>
            <TooltipItem>
              <span className="border-accent-foreground min-h-4 min-w-4 rounded-sm bg-red-600 dark:bg-red-500"></span>
              <p className="text-sm">
                <span className="font-semibold">Vencido</span> - Produto fora do prazo de validade.
              </p>
            </TooltipItem>
          </ToolTipHelp>
        ) : null}
      ...
      {addButton === true && addButtonType === TableType.USER ? (
        <AddEditDialog entity="Usuário" mode={ModeType.ADD} formComponent={FormAddUser} />
      ) : null}
      {addButton === true && addButtonType === TableType.PRODUCT ? (
        <AddEditDialog entity="Produto" mode={ModeType.ADD} formComponent={FormAddProduct} />
      ) : null}
