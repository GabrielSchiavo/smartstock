...
export const getTotalAmountByMonthChart = async (
  ...
  try {
    ...
    const products = await productRepository.findAllByReceiptDate(startDate, endDate);
    ...
    const monthlyTotals = monthNames.map((monthName, index) => {
      ...
      const totals = calculateTotals(monthProducts as unknown as CalculableTotalItemProps[]);
...
export const getProductsCountByValidityStatus =
  async (): Promise<ProductsCountByValidityStatusResponse> => {
    try {
      ...
      const validCount = productsCount - (expiredProducts.length + expiringSoonProducts.length);
